;(function ($) {
        ...
        if ($().countTo) {
            $('.roll-counter').on('on-appear', function () {
                $(this).find('.numb').each(function () {
                    ...
                    $(this).countTo({to: to, speen: speed});
                });
            });
        }
    };
    ...
    var langTrigger = function () {
        $('.top').find('.language').each(function () {
            var langTrigger = $(this).find('.lang-trigger'), langList = $(this).find('.lang-list');
            ...
            $(document).mouseup(function (e) {
                if (!langTrigger.is(e.target) && langTrigger.has(e.target).length === 0) {
                    langList.removeClass('human').addClass('ghost');
                }
            });
        });
    ...
    var galleryV1 = function () {
        $('.galleries').each(function () {
            ...
            $this.cubeportfolio({
                ...
                rewindNav: false,
                mediaQueries: [{width: 1500, cols: item}, {width: 1100, cols: item}, {width: 800, cols: 3}, {
                    width: 550,
                    cols: 2
                }, {width: 320, cols: 1}],
                caption: ' ',
        ...
        $('.roll-gallery .gallery-button a').on('click', function (e) {
            ...
            var el = $(this), url = el.attr('href'), page = parseInt(el.attr('data-page'), 10);
            el.text('Loading...');
            $.ajax({type: "GET", url: url, dataType: "html", async: false, data: {page: page}}).done(function (data) {
                if (data != null) {
                    var newitem = $(data);
    ...
    var animations = function () {
        $('.animate-scroll').each(function () {
            var $this = $(this), animateClass = $this.data('animation'), animateDelay = $this.data('animation-delay'),
            ...
            }, {triggerOnce: true, offset: animateOffset});
        });
    };
    var toggles = function () {
        ...
        $('.roll-accordion .toggle-title, .accordion .toggle-title').on('click', function () {
            if (!$(this).parent().is('.active')) {
                $(this).parent().toggleClass('active').children('.toggle-content').slideToggle(args).parent().siblings('.active').removeClass('active').children('.toggle-content').slideToggle(args);
            } else {
    ...
    var progressBar = function () {
        $('.progress-bar').on('on-appear', function () {
            $(this).each(function () {
                ...
                $(this).find('.progress-animate').animate({"width": percent + '%'}, 1000, "easeOutBack");
                $(this).parent('.roll-progress').find('.perc').addClass('show').animate({"width": percent + '%'}, 1000, "easeOutBack");
            });
        });
    };
    var tabs = function () {
            ...
            $('.roll-contact-form').each(function () {
                $(this).validate({
                    submitHandler: function (form) {
                        var $form = $(form), str = $form.serialize(), loading = $('<div />', {'class': 'loading'});
                        $.ajax({
                            type: "POST", url: $form.attr('action'), data: str, beforeSend: function () {
            ...
            $("#map").gmap3({
                map: {
                    options: {
                        ...
                        mapTypeControlOptions: {mapTypeIds: ['zupahealth_style', google.maps.MapTypeId.SATELLITE, google.maps.MapTypeId.HYBRID]},
                        scrollwheel: true
                    }
                }, getlatlng: {
                    address: "New York, NY 10017 United States", callback: function (results) {
                        if (!results) return;
                        $(this).gmap3('get').setCenter(new google.maps.LatLng(results[0].geometry.location.lat(), results[0].geometry.location.lng()));
                        $(this).gmap3({marker: {latLng: results[0].geometry.location}});
                    }
                }, styledmaptype: {id: "zupahealth_style", options: {name: "ZupaHealth Map"},},
            });
        }
    };
    var parallax = function () {
        if ($('.parallax').length > 0 && isMobile.any() == null) {
            $.stellar({horizontalScrolling: false, verticalScrolling: true, horizontalOffset: 0, verticalOffset: 0});
        }
    };
    ...
    var priceFilter = function () {
        if ($('.price-filter').find('.price-slider-range').length > 0) {
            $(".price-filter").each(function () {
                var $sliderRange = $(this).find('.price-slider-range'), $valueFrom = $(this).find('span.from'),
                    $valueTo = $(this).find('span.to');
                $sliderRange.slider({
                    range: true, min: 0, max: 20000, values: [1, 10000], slide: function (event, ui) {
    ...
    var departmentToggle = function () {
        $('.departments-list').each(function () {
            ...
            $(this).find('.toggle-title').on('click', function () {
                if (!$(this).parent().is('.active')) {
                    $(this).parent().toggleClass('active').children('.toggle-content').slideToggle('100', function () {
                        $('html, body').animate({scrollTop: $(this).parent().offset().top});
                    });
                    $(this).parent().siblings('.active').removeClass('active').children('.toggle-content').slideToggle();
                } else {
                    $(this).parent().toggleClass('active');
